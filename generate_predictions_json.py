...
DOMESTIC_LEAGUES = ["premier_league", "ligue_1", "la_liga", "serie_a"]
...
MARKET_EDGE_THRESHOLDS = {
    # 1X2 (per-league thresholds in OPTIMAL_EDGE override these)
    'home': 8.0,
    'draw': 5.0,
    'away': 15.0,
    ...
    'spread_home_m25': 5.0,
    'spread_away_p25': 5.0,
    # Asian Handicap (from spreads market — variable line from bookmaker)
    'ah_home': 8.0,
    'ah_away': 8.0,
}
...
MARKET_MIN_PROB = {
    ...
    'spread_home_m15': 0.38,
    'spread_away_p15': 0.55,
    'spread_home_m25': 0.28,
...
# Per-league edge threshold overrides (legacy, now superseded by OPTIMAL_EDGE)
LEAGUE_EDGE_OVERRIDES: dict[str, dict[str, float]] = {}

# Per-league minimum probability overrides (legacy, now superseded by OPTIMAL_MIN_PROB)
LEAGUE_MIN_PROB_OVERRIDES: dict[str, dict[str, float]] = {}

# Per-league per-market strategy: which probability source to use
...
# Optimal per-league × per-market edge thresholds and min probs.
# Generated by scripts/optimize_all.py grid search.
# When a league×market is present here, it REPLACES the global defaults.
OPTIMAL_EDGE: dict[str, dict[str, float]] = {
    ...
    "la_liga": {
        ...
        "ah_home": 20.0,
        # ah_away excluded: -18.7u in backtest
    },
    "serie_a": {
...
def train_league_models(league: str) -> LeagueModelBundle:
    ...
    # 5. ELO (with seasonal decay for contextual ratings)
    print(" Entrainement ELO...")
    elo_model = EloRating(k_factor=20, home_advantage=100)
    current_season = matches_data[0].get("season") if matches_data else None
    for md, m in zip(matches_data, matches):
        md_season = md.get("season")
        if md_season and current_season and md_season != current_season:
            elo_model.apply_seasonal_decay()
            print(f"  ELO seasonal decay: {current_season} → {md_season}")
            current_season = md_season
...
def calculate_kelly_stake(prob, odds, fraction=0.15):
...
def generate_domestic_prediction(
    ...
    # Per-league per-market strategy selection
    strategy = LEAGUE_MARKET_STRATEGY.get(bundle.league, {})
    dc_pred = dc_model.predict(home_team, away_team)

    # --- Compute all probability sources ---
    # Baseline: DC + ELO (no XGB)
    ens_base = EnsemblePredictor(
        dc_model=dc_model, elo_model=elo_model,
        dc_weight=0.65, elo_weight=0.35,
    )
    pred_base = ens_base.predict(home_team, away_team)

    # XGB sources (if fitted)
    if xgb_model.is_fitted and match_features is not None:
        ...
        pred_xgb = ens_xgb.predict(home_team, away_team, match_features=match_features)

        # xgb_draw: XGB for draw only, baseline for home/away
        ens_xgb_draw = EnsemblePredictor(
            dc_model=dc_model, elo_model=elo_model,
            dc_weight=0.65, elo_weight=0.35,
            xgb_model=xgb_model, xgb_markets={"draw"},
        )
        pred_xgb_draw = ens_xgb_draw.predict(home_team, away_team, match_features=match_features)

        # xgb_cal: XGB + isotonic calibration on 1X2
        cals = getattr(bundle, 'calibrators', {})
        xgb_h, xgb_d, xgb_a = pred_xgb.home_prob, pred_xgb.draw_prob, pred_xgb.away_prob
        if cals and 'home_win' in cals and 'draw_win' in cals and 'away_win' in cals:
            cal_h = float(cals['home_win'].predict([xgb_h])[0])
            cal_d = float(cals['draw_win'].predict([xgb_d])[0])
            cal_a = float(cals['away_win'].predict([xgb_a])[0])
            cal_total = cal_h + cal_d + cal_a
            if cal_total > 0:
                xgb_cal_probs = (cal_h / cal_total, cal_d / cal_total, cal_a / cal_total)
            else:
                xgb_cal_probs = (xgb_h, xgb_d, xgb_a)
        else:
            xgb_cal_probs = (xgb_h, xgb_d, xgb_a)
    else:
        pred_xgb = pred_base
        pred_xgb_draw = pred_base
        xgb_cal_probs = (pred_base.home_prob, pred_base.draw_prob, pred_base.away_prob)
        cals = getattr(bundle, 'calibrators', {})

    # --- Select per-market 1X2 probability ---
    source_probs = {
        "baseline": (pred_base.home_prob, pred_base.draw_prob, pred_base.away_prob),
        "xgb": (pred_xgb.home_prob, pred_xgb.draw_prob, pred_xgb.away_prob),
        "xgb_draw": (pred_xgb_draw.home_prob, pred_xgb_draw.draw_prob, pred_xgb_draw.away_prob),
        "xgb_cal": xgb_cal_probs,
    }

    home_src = strategy.get("home", "xgb")
    draw_src = strategy.get("draw", "xgb")
    away_src = strategy.get("away", "xgb")

    home_prob = source_probs.get(home_src, source_probs["xgb"])[0]
    draw_prob = source_probs.get(draw_src, source_probs["xgb"])[1]
    away_prob = source_probs.get(away_src, source_probs["xgb"])[2]

    # Renormalize (sources may not sum to 1 when mixed)
    ...
    # Apply isotonic calibration on O/U and BTTS
    if cals:
        if over15_prob is not None and 'over15' in cals:
            over15_prob = float(cals['over15'].predict([over15_prob])[0])
        if over25_prob is not None and 'over25' in cals:
    ...
    # Asian Handicap from DC score matrix (variable line from bookmaker odds)
    ah_home_prob = None
    ah_away_prob = None
    if hasattr(dc_pred, 'score_matrix') and dc_pred.score_matrix is not None and odds_entry:
        best_odds_raw = extract_best_odds(odds_entry)
        ah_entry = best_odds_raw.get("ah_home", {})
        ah_line = ah_entry.get("line") if isinstance(ah_entry, dict) else None
    ...
    extra_probs = dict(
        ...
        spread_home_m15_prob=spread_home_m15_prob, spread_away_p15_prob=spread_away_p15_prob,
        spread_home_m25_prob=spread_home_m25_prob, spread_away_p25_prob=spread_away_p25_prob,
        ah_home_prob=ah_home_prob, ah_away_prob=ah_away_prob,
    )
...
def _extract_odds_and_edge(odds_entry, home_prob, draw_prob, away_prob,
                           ...
                           ah_home_prob=None, ah_away_prob=None):
    ...
    # Asian Handicap (2-way market — use remove_margin_2way for fair probs)
    ah_home_raw = best_odds_raw.get('ah_home', {})
    ah_away_raw = best_odds_raw.get('ah_away', {})
    ah_home_odds = ah_home_raw.get('odds', 0) if isinstance(ah_home_raw, dict) else 0
    ah_away_odds = ah_away_raw.get('odds', 0) if isinstance(ah_away_raw, dict) else 0
    if ah_home_odds > 1.0 and ah_away_odds > 1.0 and ah_home_prob is not None:
        fair_ahh, fair_aha = remove_margin_2way(ah_home_odds, ah_away_odds)
        best_odds['ah_home'] = ah_home_odds
        best_odds['ah_away'] = ah_away_odds
        bookmaker['ah_home'] = ah_home_raw.get('bookmaker', 'Unknown')
        bookmaker['ah_away'] = ah_away_raw.get('bookmaker', 'Unknown')
        edge['ah_home'] = ah_home_prob - fair_ahh
        edge['ah_away'] = ah_away_prob - fair_aha
...
def _compute_recommendation(home_prob, draw_prob, away_prob, best_odds, edge,
    ...
    secondary = {
        ...
        'ah_home': ah_home_prob,
        'ah_away': ah_away_prob,
    }
    ...
    # Apply per-league optimized thresholds (OPTIMAL_EDGE/OPTIMAL_MIN_PROB take priority)
    opt_edge_map = OPTIMAL_EDGE.get(league, {})
    opt_mp_map = OPTIMAL_MIN_PROB.get(league, {})
    ...
    for market in all_markets:
        ...
        prob = probs[market]
        # Use optimizer values if available, otherwise fall back to global defaults
        if market in opt_edge_map:
            threshold = opt_edge_map[market] / 100.0
        else:
            threshold = MARKET_EDGE_THRESHOLDS[market] / 100.0
        if market in opt_mp_map:
            min_prob = opt_mp_map[market]
        else:
            min_prob = MARKET_MIN_PROB.get(market, 0.0)
...
def main():
    ...
    for fixture in fixtures:
        ...
        if pred:
            ...
            pred['home_crest'] = fixture.get('home_crest', '')
            pred['away_crest'] = fixture.get('away_crest', '')
